...
export function Login() {
	...
	const handlerSignInSuccess = (user: DataEntityType) => {
		const userPath = user.profilePath || user.id;
		navigation(`/user/${userPath}`);
	};
	const handlerSignInError = (message: string) => {
		alert.error("Вы ввели неверные данные");
		// alert.error(message);
	};

	const onSubmit: SubmitHandler<FieldsValue> = ({ email, password }) => {
		signIn(email, password, handlerSignInSuccess, handlerSignInError);
	};

	const [isShowPassword, setIsShowPassword] = useState(false);

	const isDisabledButton = !!Object.values(watch()).filter(
		(value) => value === ""
	).length;
	...
	return (
		<div className="form-entry">
			<div className="form-entry__wrapper">
				<div className="form-entry__title">Вход в Yoldi Agency</div>
				<form className="form-entry__form" onSubmit={handleSubmit(onSubmit)}>
					<Controller
						name="email"
						control={control}
						rules={{ required: true }}
					...
					<Button
						type="submit"
						theme="revers"
						className={isDisabledButton ? "button-default-revers-disabled" : ""}
					>
						Войти
					</Button>
				</form>
